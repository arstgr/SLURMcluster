...
echo 'export PATH=/share/sched/slurm/22.05.3/bin:$PATH' | sudo tee $SCHEDROOT/slurm/etc/slurm_path.sh
...
cd pyxis-0.11.1
sudo CFLAGS='-I/share/sched/slurm/22.05.3/include' prefix=$SCHEDROOT/slurm/22.05.3 make install
cd ..
...
echo 'required /share/sched/slurm/22.05.3/lib/slurm/spank_pyxis.so' | sudo tee $SCHEDROOT/slurm/etc/plugstack.conf.d/pyxis.conf
